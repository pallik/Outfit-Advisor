...
- (UIImage *) resizedImageByMagick: (NSString *) spec
{
    ...
    if([spec hasSuffix:@"!"]) {
        ...
        NSUInteger width = (NSUInteger)[[widthAndHeight objectAtIndex: 0] longLongValue];
        NSUInteger height = (NSUInteger)[[widthAndHeight objectAtIndex: 1] longLongValue];
    ...
    if ([widthAndHeight count] == 1) {
        return [self resizedImageByWidth: (NSUInteger)[spec longLongValue]];
    }
    if ([[widthAndHeight objectAtIndex: 0] isEqualToString: @""]) {
        return [self resizedImageByHeight: (NSUInteger)[[widthAndHeight objectAtIndex: 1] longLongValue]];
    }
...
//    UIGraphicsBeginImageContext(self.size);
	[self beginImageContextForSize:self.size];
...
//    UIGraphicsBeginImageContext(bounds.size);
	[self beginImageContextForSize:bounds.size];
...
//    UIGraphicsBeginImageContext(rect.size);
	[self beginImageContextForSize:rect.size];
    CGContextRef context = UIGraphicsGetCurrentContext();
...
- (void)beginImageContextForSize:(CGSize)size
{
